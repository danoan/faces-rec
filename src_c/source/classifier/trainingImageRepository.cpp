...
void TrainingImageRepository::init(int features_number){
    ...
    */
    ...
    _max_buffer = Config::INTEGRAL_IMAGE_BUFFER_SIZE*(100000/features_number);
...
void TrainingImageRepository::addFace(std::string imagePath, TrainingType tit){
    if(_faces.size()<_max_buffer){
        _faces.push_back(new TrainingImage(imagePath,tit,Config::HAS_BUFFER,_features_number)); 
    }else{
